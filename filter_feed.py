...
def fetch_and_filter():
    ...
    # Parsa l'XML grezzo — nessun feedparser intermedio
    root = ET.fromstring(resp.content)
    channel_in = root.find("channel")
    if channel_in is None:
        raise RuntimeError("Feed non valido: elemento <channel> non trovato")

    # Costruisce il nuovo RSS
    ...
    ET.SubElement(channel_out, "title").text       = "Il Post [filtrato]"
    ET.SubElement(channel_out, "link").text        = "https://www.ilpost.it"
    ET.SubElement(channel_out, "description").text = f"Feed filtrato — categorie: {', '.join(WHITELIST)}"
    ET.SubElement(channel_out, "language").text    = "it-IT"
    ET.SubElement(channel_out, "lastBuildDate").text = datetime.now(timezone.utc).strftime(
        "%a, %d %b %Y %H:%M:%S +0000"
    )
    ...
    for item_in in channel_in.findall("item"):
        # Legge TUTTE le categorie dall'XML grezzo
        categories = {
            (cat.text or "").strip().lower()
            for cat in item_in.findall("category")
        }

        # Basta che ALMENO UNA categoria sia in whitelist
        ...
        def cp(tag, dest_tag=None):
            ...
            if el is not None and el.text:
                ET.SubElement(item_out, dest_tag or tag).text = el.text.strip()
        ...
        if guid_el is not None:
            ...
            g.set("isPermaLink", guid_el.get("isPermaLink", "false"))

        for cat_el in item_in.findall("category"):
            if cat_el.text:
                ET.SubElement(item_out, "category").text = cat_el.text.strip()

        # dc:creator (opzionale)
        creator = item_in.find(f"{{{NS['dc']}}}creator")
        if creator is not None and creator.text:
            dc_el = ET.SubElement(item_out, "dc:creator")
